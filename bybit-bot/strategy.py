...
Pure Price Action: Higher Low Trendline + Flat Resistance + Ascending Triangle
...
# ══════════════════════════════════════════════════════════════
# WICK-BASED PIVOT & HIGHER LOW DETECTION (for Fibonacci LONG)
# Mirror kualitas dari detect_lower_highs (SHORT) tapi kebalikan
# ══════════════════════════════════════════════════════════════

def detect_pivot_lows_wick(df: pd.DataFrame, left: int = PIVOT_LEFT,
                           right: int = PIVOT_RIGHT) -> List[int]:
    """
    Detect pivot low points using WICK low price (df['low']).
    Mirror dari detect_pivot_highs yang juga pakai wick (df['high']).
    Wick-based detection lebih sensitif dan konsisten dengan cara trader
    menandai swing low di chart — pakai shadow/wick, bukan body.
    """
    lows = df['low'].values
    n = len(lows)
    pivots = []
...
def detect_higher_lows_wick(df: pd.DataFrame, p_lows: List[int]) -> Tuple[bool, List[int]]:
    """
    Detect Higher Lows pattern — STRICT v2 (wick-based).
    MIRROR EXACT dari detect_lower_highs tapi KEBALIKAN:
    - Setiap pivot low harus LEBIH TINGGI dari sebelumnya
    - Gap antar HL: minimal MIN_HL_CANDLE_GAP, maksimal MAX_HL_CANDLE_GAP
    - Price jump antar HL tidak boleh > MAX_HL_PRICE_JUMP_PCT (outlier filter)
    - Pilih BEST sequence dari semua kemungkinan starting point

    Menggunakan df['low'] (wick) untuk konsistensi dengan detect_lower_highs
    yang juga pakai df['high'] (wick).
    """
    ...
    for start in range(len(p_lows)):
        ...
        for i in range(start + 1, len(p_lows)):
            ...
            # Outlier filter: lonjakan harga antar HL tidak boleh terlalu besar
    ...
    # Recency check: HL terakhir harus dalam 80 candle terakhir
...
def analyze_asc_triangle_long(df: pd.DataFrame, symbol: str, timeframe: str) -> Optional[Dict]:
    ...
    TIDAK ADA: stochastic check, consolidation check
    ...
    try:
        ...
        # Anti-Nukik Logic: Ensure slope is <= ~60 degrees (1.2% per candle max)
        # 0.5% was too strict — BSB(0.75%), APE(0.57%), BIO(0.69%) all valid ascending triangles
        if slope_pct_per_candle > 1.2:
        ...
        for k in range(first_hl_idx, len(df)):
            ...
            # Harga menyentuh zona resistance (dari bawah) berdasarkan wick high
            if candle_wick_high >= flat_resistance_level - resistance_tolerance:
        ...
        if not trendline_price or trendline_price <= 0:
            return None  # Tidak ada trendline yang valid
...
def analyze_lh_short(df: pd.DataFrame, symbol: str, timeframe: str) -> Optional[Dict]:
    """
    Kalimasada v7.2 — FIBONACCI RETRACEMENT SHORT
    ...
    try:
        ...
        # ═══ STEP 3: RESISTANCE TRENDLINE (reference) ═══
        ...
        # ═══ STEP 4: FIBONACCI RETRACEMENT — Swing HIGH → Swing LOW ═══
        #
        # Cara manual di TradingView (SHORT entry):
        ...
        for i in range(len(lh_indices) - 1, -1, -1):
            ...
            # Cari swing low SETELAH LH ini (lowest low dari LH sampai current-1)
            search_start = candidate_high_idx + 1
            search_end = current_idx  # Exclude current candle

            if search_end - search_start < 2:
                continue  # Not enough candles after this LH
            ...
            # Swing low must not be the last candle (need bounce confirmation)
            if candidate_low_idx >= current_idx - 1:
                continue

            # Price must have bounced UP from swing low
            ...
            # Minimum: drop harus >= 3% untuk meaningful Fib
...
def is_pucuk(df: pd.DataFrame) -> bool:
    """
    Pucuk Protector v5 — PURE PRICE ACTION. No indicators.
    Detect if price is at peak/pump.
    Used on H4 and D1 timeframes to block entries.

    Returns True if PUCUK (should REJECT entry) if ANY of:
    1. Price > 7% above SMA-20 (Overextended)
    2. ANY of last 5 candles has body > 2.5x ATR (Pump candle)
    3. Price rose > 5% in last 5 candles (Rapid rise)
    4. Price at 14-period high (near ceiling)
    """
    ...
    # Check 1: SMA distance — harga terlalu jauh di atas rata-rata
    ...
    # Check 2: Price at 14-period high (pure PA overbought)
    # Jika harga = highest high dalam 14 candle terakhir, itu sudah pucuk
    if len(df) >= 14:
        highest_high_14 = df['high'].iloc[-14:].max()
        if current_price >= highest_high_14 * 0.995:
            return True  # PUCUK: Price at/near 14-period ceiling
