...
# ══════════════════════════════════════════════════════════════════
# PROFESSIONAL TRADE SETUP FORMATTING
# ══════════════════════════════════════════════════════════════════

def fmt_trade_setups(setups: List[Dict]) -> str:
    """Format professional trade ideas (fundamental + technical + anomaly)."""
    if not setups:
        return "🚫 Tidak ada trade setup berkualitas tinggi."

    lines = [
        "🎯 <b>PROFESSIONAL SWING SETUP</b>",
        f"<i>Berdasarkan {len(setups)} anomali yg difilter Fundamental & Teknikal</i>",
        "",
    ]
    ...
    for i, s in enumerate(setups[:5], 1):
        ...
        # Risk assessment
        risk_emoji = "🟢" if score >= 85 else "🟡" if score >= 70 else "🟠"
        
        # Data objects
        setup = s.get("setup", {})
        signals = s.get("technical_signals", [])
        
        # Build text 
        lines.append(f"{i}. {risk_emoji} <b>{ticker}</b> (Conviction: {score}%)")
        lines.append(f"   📈 <b>Trend:</b> {s.get('technical_trend', 'Unknown')}")
        
        for sig in signals:
            lines.append(f"   • {sig}")
            
        lines.append("")
        lines.append(f"   💼 <b>Trade Setup:</b>")
        lines.append(f"   🟢 Entry: {setup.get('entry_zone', 'N/A')}")
        lines.append(f"   🎯 TP: {setup.get('tp1', 0):,.0f} - {setup.get('tp2', 0):,.0f}")
        lines.append(f"   🛑 SL: {setup.get('sl', 0):,.0f}")
        lines.append(f"   ⚖️ R/R: <b>{setup.get('risk_reward_ratio', 0)}x</b>")
        
        lines.append("")
        lines.append(f"   🛡 <b>Garbage Filter:</b> {s.get('fundamental_safety', '')}")
        lines.append("─" * 30)

    lines.append("<i>Disclaimer: Trade ini memiliki probabilitas, bukan kepastian. Selalu pasang Stop Loss.</i>")
    return "\n".join(lines)
